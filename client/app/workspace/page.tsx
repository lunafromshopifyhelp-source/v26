...
export default function Workspace() {
  ...
  const fetchVaultAndProfile = async () => {
    ...
    try {
      ...
      const postsRes = await axios.get(`http://localhost:5000/api/posts/bridge/${email}/${res.data.partnerEmail || 'none'}`);
  ...
  const handleInitialize = async () => {
    ...
    const email = localStorage.getItem('v26UserEmail');
    try {
      await axios.post('http://localhost:5000/api/missions/create-mission', {
        creatorEmail: email,
        title: newMission,
        timeframe: timeframe
      });
  ...
  const linkPartner = async () => {
    ...
    try {
      setIsLinking(true);
      await axios.put('http://localhost:5000/api/auth/link-partner', { myEmail, partnerEmail });
  ...
  const acceptMission = async () => {
    const myEmail = localStorage.getItem('v26UserEmail');
    try {
      await axios.put('http://localhost:5000/api/auth/accept-mission', { 
        myEmail, 
